...
public class BlendAnimatorSpriteStudio : MonoBehaviour
{
    ...
    public AnimationCurve interpolateCurve = AnimationCurve.Linear(0, 0, 1, 1);
    [SerializeField]
    private float depthPowMult = 2.0f;
    [SerializeField]
    private float depthPowAdd = 0.5f;
    ...
    public void GenerateTransitionAnimation(int targetAnimationIndex)
    {
        ...
        GenerateAnimation(fromPoseCache, toPoseCache, generateFrameCount, defaultAnimationIndex);
    ...
    private void GenerateAnimation(PoseInfo fromPose, PoseInfo toPose, int frameCount, int copyAnimationIndex)
    {
        ...
        for (int i = 0; i < partLen; i++)
        {
            ...
            float depthPower = (treeDepthCache[i] / maxDepth) * depthPowMult + depthPowAdd;

            for (int f = 0; f < frameCount; f++)
            {
                weightCache[f] = Mathf.Pow(interpolateCurve.Evaluate(f_div * f), depthPower);
            }
